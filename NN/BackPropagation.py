...
class NeuralNetwork:
    ...
    def train(self, training_inputs, training_outputs):
        ...
        # 4. 更新隐含层的权重系数
        for h in range(len(self.hidden_layer.neurons)):
            for w_hi in range(len(self.hidden_layer.neurons[h].weights)):
                # ∂Eⱼ/∂wᵢ = ∂E/∂zⱼ * ∂zⱼ/∂wᵢ
                pd_error_wrt_weight = pd_errors_wrt_hidden_neuron_total_net_input[
                    h]*self.hidden_layer.neurons[h].calculate_pd_total_net_input_wrt_weight(w_hi)

                # Δw = α * ∂Eⱼ/∂wᵢ
                self.hidden_layer.neurons[h].weights[w_hi] -= self.LEARNING_RATE * \
                    pd_error_wrt_weight
