...
def data_review():
    ...
    # 绘制句子长度及出现频数统计图,这对后面的建模时填充的句子长度有有参考作用。
    # 按句子长度进行排序
    sort_sent_len_dist = sorted(
        dict(Counter(sent_len_list)).items(), key=itemgetter(0))
    ...
    # 绘制句子长度累积分布函数(CDF)
    sent_pentage_list = [(count/sent_num)
                         for count in accumulate(sent_count_data)]
...
def data_processing():
    input_data = load_data()
    # 标签及词汇表
    labels, vocabulary = list(input_data['tag'].unique()), list(
        input_data['word'].unique())
    ...
    dict_list = [word_dictionary, inverse_word_dictionary,
                 label_dictionary, output_dictionary]
