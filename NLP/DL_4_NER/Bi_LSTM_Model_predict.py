...
def input_data_for_model(input_shape):
    ...
    y = [np_utils.to_categorical(
        label, num_classes=label_size+1) for label in y]
...
def model_train():
    ...
    # 模型训练
    lstm_model = create_Bi_LSTM(
        vocab_size, label_size, input_shape, output_dim, n_units, out_act, activation)
    lstm_model.fit(train_x, train_y, epochs=epochs,
    ...
    for start, end in zip(range(0, N, 1), range(1, N+1, 1)):
        ...
        output_sequences = ' '.join(
            [output_dictionary[key] for key in output_sequences if key != 0]).split()
        input_sequences = ' '.join([output_dictionary[key]
                                    for key in input_sequences if key != 0]).split()
